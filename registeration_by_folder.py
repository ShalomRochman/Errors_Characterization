...
class liver_registeration():
    ...
    def registeration(self,trans_type='translation',liver_mask=True):
        for i, BL_Scan in enumerate(self.CT_BL):
            ...
            if trans_type=='affine' or trans_type=='bspline':
                self.fix_origin(Baseline,FollowUp)
                self.fix_origin(Baseline,FollowUp_Liver)
                self.fix_origin(Baseline,Baseline_Liver)
                self.fix_origin(Baseline,Baseline_Liver)
                self.fix_origin(Baseline,Baseline_Tumors)
                self.fix_origin(Baseline,FollowUp_Tumors)
            ...
            try:
                ...
                os.symlink(FollowUp, self.dest_path + '/' + case_name + '/FU_Scan_CT.nii.gz')
                os.symlink(FollowUp_Liver, self.dest_path + '/' + case_name + '/FU_Scan_Liver.nii.gz')
                os.symlink(FollowUp_Tumors, self.dest_path + '/' + case_name + '/FU_Scan_Tumors.nii.gz')
            except:
    ...
    def affine_registeration(self):
        ...
        try:
            ...
            try:
                print('Trying to register by affine')
